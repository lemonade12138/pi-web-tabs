...
function persistOpenTabIds(ids: string[]) {
  try { window.localStorage.setItem(OPEN_TABS_KEY, JSON.stringify(ids)); } catch {}
}
...
export function AppShell() {
  ...
  // Top strip tabs work like browser tabs: opening a session anywhere (sidebar,
  // strip, restore, fork) pins a tab; × removes only the tab, never the session.
  // null = not seeded yet → the strip falls back to the most recent sessions.
  const [openTabIds, setOpenTabIds] = useState<string[] | null>(null);
  const ensureTab = useCallback((sessionId: string) => {
    setOpenTabIds((prev) => {
      // activation never reorders tabs — clicking only highlights; new tabs append
      if (prev && prev.includes(sessionId)) return prev;
      let next: string[];
      if (prev) {
        next = [...prev, sessionId].slice(-50);
      } else {
        // first interaction seeds the strip: clicked session + current top recents
  ...
  // Tab strip attention: sessions that finished running since last viewed
  ...
  const handleCwdChange = useCallback((
    ...
    // Tab-switch flow: handleSelectSession already opened the target session,
    // and it belongs to the incoming project — keep it open instead of
    // resetting to a fresh composer while the workspace context catches up.
    if (selectedSession && workspaceKeyOf(selectedSession) === newProject) {
      return;
    }
  ...
  }, [invalidateWorkspaceRestore, router, isMobile, selectedSession, ensureTab]);
  ...
  // Top strip sessions: the open-tab list when seeded, recent sessions otherwise
  const recentSessions = useMemo(() => {
    const catalogById = new Map(sessionCatalog.map((s) => [s.id, s] as const));
    ...
    // keep the selected session visible even if it is not pinned as a tab
    if (selectedSession && !base.some((s) => s.id === selectedSession.id)) {
      base.unshift(selectedSession);
    }
    // draft tab only when a new-session flow is actually active (+ button / fork),
    // not on every transient moment where selection is briefly empty
    if (newSessionDraftId !== "initial" && newSessionCwd && !selectedSession) {
    ...
    return base;
  }, [sessionCatalog, selectedSession, newSessionDraftId, newSessionCwd, openTabIds]);
  ...
  const handleTabCreate = useCallback(() => {
    if (!activeCwd) return;
    handleNewSession(`tab-${Date.now().toString(36)}`, activeCwd);
  }, [activeCwd, handleNewSession]);

  const handleTabClose = useCallback((sessionId: string) => {
    // hide from the strip only — the session itself is never deleted here
    closeTab(sessionId);
  }, [closeTab]);
